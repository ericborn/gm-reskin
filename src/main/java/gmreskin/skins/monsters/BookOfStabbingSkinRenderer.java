...
public class BookOfStabbingSkinRenderer extends SkinRenderer {
    public BookOfStabbingSkinRenderer(String skinPath) {
        super(skinPath);
    ...
    public void onChangeState(String stateName) {
        switch (stateName) {
            case "ATTACK":
                this.animation.setCurAnimation("multiAttack");
                break;
            case "ATTACK_2":
                this.animation.setCurAnimation("stab");
        }
    }

    @Override
    public void onDamaged(int damageAmount) {
        if (this.animation.getCurAnimationName().equals("idle") || this.animation.getCurAnimationName().equals("hit")) this.animation.setCurAnimation("hit");
    }
}
